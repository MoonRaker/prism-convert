...
def import_data(directory, afile):
    ...
    raw_data = np.zeros((len(site_names), 2, 12))

    for i in range(len(site_names)):
        raw_data[i,0,:] = [float(ws.Cells(i+2,col).Value) for col in range(5,17)]
        raw_data[i,1,:] = [float(ws.Cells(i+2,col).Value) for col in range(18,30)]

    dp = pd.Panel(raw_data, items=site_names, major_axis=['p', 't'])
...
def convert_data(dp):
    ...
    cdata_dict = {}
    for i in range(len(site_names)):
        cdata_dict[site_names[i]] = pd.DataFrame([precips[i], temps[i]], index=['p', 't'])
    ...
    for item in cdata.items:
        ...
        if flag:
            ...
            for ind in range(12):
                ...
                ws.Cells(row,4).Value = int(cdata[item].loc['t'].values[ind])       
